		...
		if result.DeletedCount < 1 {
			c.JSON(http.StatusNotFound,
				responses.UserResponse{
					...
					Message: "error", Data: map[string]interface{}{"data": "User with specified ID not found!"},
				},
			)
			return
		}
...
func GetWelcomeMessage() gin.HandlerFunc {
	return func(c *gin.Context) {
		//c.JSON(http.StatusOK, gin.H{"config": configs.GetConfig()})
		c.JSON(http.StatusOK, gin.H{"message": "Welcome bicho!"})
	}
}
